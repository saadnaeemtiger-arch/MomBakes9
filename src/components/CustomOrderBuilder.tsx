  ...
  return (
    <section id="custom-builder" className="py-20 md:py-28 bg-cream border-t border-b border-cream-dark">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-20 relative">
          ...
          {steps.map((step, idx) => (
            ...
            >
              ...
              <div className="relative w-14 h-14 rounded-full bg-white border border-pastel-pink-dark shadow-sm flex items-center justify-center group hover:border-warm-brown hover:bg-pastel-pink transition-all duration-300">
                {step.icon}
                <span className="absolute -top-1 -right-1 w-5.5 h-5.5 bg-warm-brown text-cream font-display text-[10px] font-bold rounded-full flex items-center justify-center border border-white">
                  {step.num}
                </span>
              </div>
              ...
              <div className="space-y-1 max-w-[180px]">
                ...
                <p className="font-sans text-[11px] text-chocolate/70 leading-relaxed font-light">{step.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-cream-dark shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0">
          
          {/* Left panel: Form configuration */}
          <div className="lg:col-span-7 p-6 sm:p-10 lg:p-12 space-y-8">
            <div className="space-y-2">
              <span className="font-display text-[10px] font-bold text-warm-brown uppercase tracking-widest bg-pastel-pink px-3 py-1 rounded-full border border-pastel-pink-dark">
                Interactive Lab
              </span>
              <h3 className="font-serif text-2xl sm:text-3xl font-bold text-chocolate pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {/* Tiers Selector */}
              <div className="space-y-2">
                <label className="block text-xs font-display font-bold uppercase tracking-wider text-chocolate">
                  Cake Tiers:
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {[1, 2, 3].map((t) => (
                    <button
                      key={t}
                      onClick={() => setTiers(t)}
                      className={`py-2 rounded-xl border text-xs font-display font-bold transition-all focus:outline-none ${
                        tiers === t
                  ...
                  {['Gluten-Free', 'Vegan', 'Eggless', 'Dairy-Free'].map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-[10px] font-display font-bold tracking-wide transition-all focus:outline-none ${
                          selected
                            ? 'bg-pastel-pink border-pastel-pink-dark text-warm-brown'
            ...
            <div className="space-y-6 relative z-10">
              ...
              <div className="py-2 flex justify-center items-center">
                <div className="flex flex-col items-center justify-end space-y-1 h-36 w-full max-w-[180px]">
                  ...
                  <AnimatePresence>
                    {tiers >= 3 && (
                      <motion.div
                        ...
                        className="w-20 h-8 rounded-t-xl bg-pastel-pink border-2 border-white flex items-center justify-center text-warm-brown font-display text-[9px] font-bold shadow-md uppercase"
                      >
                        Tier 3
                      </motion.div>
                    )}
                  ...
                  <AnimatePresence>
                    {tiers >= 2 && (
                      <motion.div
                        ...
                        animate={{ scale: 1, opacity: 1, y: 0 }}
                        exit={{ scale: 0, opacity: 0 }}
                        className="w-28 h-10 bg-pastel-pink-dark border-2 border-white flex items-center justify-center text-warm-brown font-display text-[10px] font-bold shadow-md uppercase"
                      >
                        Tier 2
                      </motion.div>
                    )}
                  ...
                  <motion.div
                    layout
                    className="w-36 h-12 rounded-b-xl bg-cream border-2 border-white flex items-center justify-center text-chocolate font-display text-xs font-bold shadow-md uppercase"
                  >
                    Base Tier
                  </motion.div>
                </div>
            ...
            <div className="pt-6 border-t border-cream/15 relative z-10 space-y-4">
              <div className="flex items-end justify-between">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-display font-semibold tracking-wider text-cream/60">Estimated Starting Quote</span>
                  <span className="font-serif text-3xl font-bold text-gold">${calculatePrice()}</span>
                </div>
                <div className="flex items-center space-x-1 text-cream/50 text-[10px] uppercase font-display">
              ...
              <button
                ...
                disabled={isAdded}
                className={`w-full py-3.5 rounded-full font-display text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center space-x-2 shadow-md ${
                  isAdded
                    ? 'bg-gold text-chocolate border border-gold-dark scale-98'
